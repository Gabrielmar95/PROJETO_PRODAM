...
**Atualizado automaticamente em 21/05/2026 06:38 via auto_update_claude_md.py**
...
- 🟠 SSP: 2026-06-30 (40 dias) — R$ 4.553.230,80
...
## AGENT SKILLS (skills de engenharia)
Convenções que skills genéricas (`to-issues`, `triage`, `to-prd`, `tdd`, `diagnose`, `grill-with-docs`) consomem neste repo. Detalhes em `docs/agents/*.md`.

- **Issue tracker** → `_QUESTOES_CRITICAS/` (arquivos `NN_TITULO.md` numerados). Ver `docs/agents/issue-tracker.md`.
- **Triage labels** → 5 canônicos (`needs-triage`/`needs-info`/`ready-for-agent`/`ready-for-human`/`wontfix`), aplicados como linha `Status:` no topo do arquivo. Ver `docs/agents/triage-labels.md`.
- **Domain docs** → single-context: `CONTEXT.md` + `docs/adr/` no root (criados sob demanda por `grill-with-docs`). Ver `docs/agents/domain.md`.
